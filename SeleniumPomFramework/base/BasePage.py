...
from SeleniumPomFramework.utilities.CustomLogger import customLogger as cl
import allure
...
class BaseClass:
    ...
    def launchWebPage(self, url, title):
        try:
            ...
            self.log.info(f"Web Page NOT Launched with URL : {url}")
            print_stack()
            self.takeScreenshot(title)
            #if it's in exception statement and fail the test case
            assert False
    ...
    def getListOfWebElements(self, locatorValue, locatorType="id"):
        webElements = None
        try:
            locatorType = locatorType.lower()
            locatorByType = self.getLocatorType(locatorType)
            webElements = self.driver.find_elements(locatorByType, locatorValue)
            self.log.info("List of WebElements found with locator value " + locatorValue + " using locatorType " + locatorByType)
        except:
            self.log.error(
                "List of WebElements not found with locator value " + locatorValue + " using locatorType " + locatorType)
            print_stack()
            self.takeScreenshot(locatorType)
            # if it's in exception statement and fail the test case
            assert False
